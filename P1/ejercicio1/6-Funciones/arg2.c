 ...
 * 1. ¿Por qué cambia el valor de 'y' tras la llamada a sum()?
 ...
 * 
 * 1. Porque lo que pasas por parametro es el puntero que apunta a la direccion de memoria de y, al modificar lo que apunta cambia el valor
 * 2. Depende de si se trata de un puntero a un struct o de una variable que almacena un struct, en el primer caso se utiliza "->" y en el 
 * 	  segundo se utiliza "."
 * 3. Como al puntero zc le estamos pasando la dirección de memoria de una varible local que se destruye al terminar la función sumC, 
 * 	  cuando volvemos a usar zc, no apunta a nada.
 * 4. Hecho
 *  
 *******************/
